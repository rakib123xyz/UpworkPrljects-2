...
userName = "asda.mcgrocer@gmail.com"
...
def UploadByMethod1(userName,passWord,loginPageUrl,uploadPageUrl):
    ...
    all_files = os.listdir("mcgrocer_temp/")
    csv_files = list(filter(lambda f: f.endswith('.csv'), all_files))
    print(csv_files)

    for item in csv_files:
        print("Trying to upload " + item)
        with open("mcgrocer_error/log.txt", mode="a") as log:
            file = log
            file.write("\n Trying to upload " + item)
            file.close()

        driver.get(upload_page_url)

        path = "mcgrocer_temp/" + item
        absPath = r"C:\Users\Rakib\PycharmProjects\UpworkProjectHelper\imageDownloader\Image link extractor\Mcgrocer/"
        ...
        if className.find("enabled-btn"):
            try:
                ...
                shutil.move(productsCsv, "mcgrocer_done/")
                print("Submited successfully")

            except Exception:
                errText = driver.find_element('xpath', '//div[@id="csv_err"]').text
                shutil.move(productsCsv, "mcgrocer_error/")
                print("Error in csv")
                with open(file="mcgrocer_error/error " + item, mode="x") as errorfile:
